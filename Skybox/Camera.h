...
public :
	...
	// Process input recieved from keyboard input system
	void ProcessKeyboard(Camera_Movement direction, GLfloat deltaTime)
	{
		...
		// Make user stay at ground level
		//Position.y = 0.0f;
	}

	// Process input received from mouse input system
	void ProcessMouseMovement(GLfloat xoffset, GLfloat yoffset, GLboolean constrainPitch = true)
	{
		...
		// Make sure if pitch is out of bounds, screen doesn't get flipped
		if (constrainPitch) {
			if (this->Pitch > 89.0f)
				this->Pitch = 89.0f;
			if (this->Pitch < -89.0f)
